...
//   //   const monthLabels = []; // Labels for x-axis (months)
//   // const monthData = []; // Data for y-axis (average testing days)
//   // Object.keys(monthAverages).forEach((monthYearKey) => {
//   //   const [month, year] = monthYearKey.split('-'); // Split the key into month and year
//   //   const monthName = this.getMonthName(month); // Convert month number to name
//   //   const average = monthAverages[monthYearKey].sum / monthAverages[monthYearKey].count;
//   //   monthLabels.push(`${monthName} ${year}`); // Include year in the label
...
//                 </TableHead>
//                 <TableBody>
//                   {products.sort((a, b) => new Date(b.reqDate) - new Date(a.reqDate)).slice(0, 5).map((product, index) => (
//                     <TableRow key={index}>
...
class AdminDashboard extends Component {
  ...
  &nbsp;<Button variant="contained" color={this.state.tableFilter === 'In Progress' ? 'primary' : 'default'} onClick={() => this.handleTableFilterChange('In Progress')}>In Progress</Button>&nbsp;
  &nbsp;<Button variant="contained" color={this.state.tableFilter === 'Pending' ? 'primary' : 'default'} onClick={() => this.handleTableFilterChange('Pending')}>Pending</Button>
</div>
